...
const BlogSection = () => {
  ...
  const cardVariants = { hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0, transition: { duration: 0.6, ease: "easeOut" } } };

  return (
    <section className="px-8 lg:px-20 py-16 bg-[#1a1a2e]">
      <motion.h2 initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="text-3xl font-bold text-[#00a8ff] mb-8">
        Latest Blogs
      </motion.h2>

      {/* Initial 8 blogs */}
      <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8" variants={containerVariants} initial="hidden" whileInView="show" viewport={{ once: true }}>
      ...
      {/* Show more blogs */}
      {visibleCount > 8 && (
        <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mt-8" variants={containerVariants} initial="hidden" animate="show">
      ...
      {visibleCount < blogs.length && (
        <div className="flex justify-center mt-8">
          <button onClick={showMore} className="bg-[#00a8ff] text-white px-6 py-3 rounded-lg shadow-lg hover:bg-[#0090dd] transition-all">
            Show More
          </button>
        </div>
      )}
